...
scenario <- rep(1:4, each = n_sim)
data_set_number <- seq_along(scenario)

# for debugging
#debug(SurvEffTox_sim)
#simulate_data(scenario = scenario[1018], data_set_number = data_set_number[1018], keep_dose0 = TRUE)

RNGkind("L'Ecuyer-CMRG")
set.seed(1)
data_sets <- mcmapply(failwith(NA, quiet(simulate_data), quiet = FALSE), 
  scenario        = scenario, 
...
save(data_sets, file = "./RData/data_sets.RData")
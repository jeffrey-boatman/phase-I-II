...
args <- expand.grid(
  which_sims = which_sims, 
  w          = w)

which_sims <- args$which_sims
prior_weights <- as.list(args$w)
...
#   mc.cores = n_cores,
#   SIMPLIFY = FALSE)
# apply_mem(data_sets[[1]], 0)

sim_out <- mcmapply(FUN = failwith(NA, quiet(apply_mem)),
  data_set     = data_sets[which_sims],
...
write.table(sim_out,
  file      = paste0("./output/mem_out_", which_serv, ".txt"),
  quote     = FALSE,
  row.names = FALSE)
